...
export function compute(currentDate, selectedDate, view, locale, weekStart) {
	...
	if (view === MODE_YEAR) {
		...
		if (
			selectedDate.getUTCFullYear() - initYear >= 0 &&
			selectedDate.getUTCFullYear() - initYear <= 2
		) {
			selectionMark =
				selectedDate.getUTCMonth() + (selectedDate.getUTCFullYear() - initYear || 0) * 12;
		}
		return {
			grid,
			todayMark,
	...
	while (prevMonth.valueOf() < nextMonthValue) {
		...
		if (
			prevMonth.getUTCFullYear() < y ||
			(prevMonth.getUTCFullYear() === y && prevMonth.getUTCMonth() < m)
		) {
			prevTo = inc;
		} else if (
			nextFrom === 42 &&
...
export function parseDate(date, format, i18n, type) {
	...
	let parts = (date && date.toString().match(formatHelper.nonpunctuation)) || [],
...
export function formatDate(date, format, i18n, type) {
	...
	if (type === 'standard') {
		...
		val = {
			...
			S:
				date.getUTCDate() % 10 <= i18n.suffix.length
					? i18n.suffix[(date.getUTCDate() % 10) - 1]
					: '',
			// hour
