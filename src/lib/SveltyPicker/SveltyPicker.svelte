...
<script>
	...
	import { usePosition } from '$lib/internal';
				...
				<div class="std-btn-row">
					{#if todayBtn}
						<button
							...
							disabled={startDate > formatDate(new Date(), format, i18n, formatType)}
							>{i18n.todayBtn}</button
						>
					{/if}
					{#if clearBtn}
						<button on:click={onClear} class={clearBtnClasses} disabled={!innerDate}
							>{i18n.clearBtn}</button
						>
					{/if}
				</div>
			{/if}
