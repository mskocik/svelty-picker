<script>
	...
	import { UTCDate } from '$lib/dateUtils';
	...
	$: handCss = isMinuteView
		...
		: `transform: rotateZ(${(selectedHour % 12) * 30}deg); ${
				selectedHour > 12 || !selectedHour ? 'height: calc(25% + 1px)' : ''
		  }`;
	...
	function onClick(e) {
		if (!canSelect) return;
		if (
			(e.type === 'mousemove' && !handleMoveMove) ||
			(!isMinuteView && e.target.tagName !== 'BUTTON')
		)
			return;
...
<div class="sdt-timer" in:fade={{ duration: 200 }}>
	<div class="sdt-time-head">
		...
		<button
			...
			on:click={() => (isMinuteView = false)}>{view(selectedHour, showMeridian)}</button
		>
		<span>:</span>
		<button
			class="sdt-time-btn sdt-time-figure"
			class:is-active={isMinuteView}
			on:click={() => (isMinuteView = true)}>{view(selectedMinutes, false)}</button
		>
		{#if showMeridian}
			<div class="sdt-meridian">
				<button
					class="sdt-time-btn"
					class:is-active={selectedHour < 12}
					on:click={onSwitchMeridian}
					data-value={selectedHour % 12}>AM</button
				>
				<button
					class="sdt-time-btn"
					class:is-active={selectedHour >= 12}
					on:click={onSwitchMeridian}
					data-value={(selectedHour % 12) + 12}>PM</button
				>
			</div>
		{/if}
	</div>
	<div
		class="sdt-clock"
		on:click={onClick}
		on:mousedown={onToggleMove}
		on:mousemove={(e) => {
			handleMoveMove && onClick(e);
		}}
		on:mouseup={onToggleMove}
		bind:this={clockEl}
		class:is-minute-view={isMinuteView}
	>
		<div class="sdt-middle-dot" />
		<div class="sdt-hand-pointer" style={handCss}>
			<div class="sdt-hand-circle" />
		</div>
		...
		{#if !showMeridian && !isMinuteView}
			{#each innerHours as p, i}
				<button
					...
					transition:fade|local={{ duration: 200 }}
					data-value={p.val}
					class:is-selected={isSelected(selectedHour, p.val, i)}>{p.val}</button
				>
			{/each}
		{/if}
	</div>
