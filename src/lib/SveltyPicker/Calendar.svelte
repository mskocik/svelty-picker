<script lang="ts">
	...
	import { compute, MODE_MONTH, MODE_YEAR, MODE_DECADE, moveGrid, UTCDate } from '$lib/dateUtils';
	import { scale } from '$lib/internal';
		...
		switch (key) {
			...
			case 'ArrowUp':
				...
				if (pos.y === 5) {
					const tmpDate = new Date(
						activeDate.getUTCFullYear(),
						activeDate.getMonth() > 0 ? activeDate.getMonth() : 11,
						1
					);
					const tmpData = compute(tmpDate, internalDate, currentView, i18n, weekStart);
					pos.y =
						tmpData.grid[5][pos.x].getUTCDate() === internalDate.getUTCDate()
							? 4
	...
	$: swapTransition = viewDelta === -2 ? fade : viewDelta !== null ? scale : () => {};
...
<div class="sdt-thead-nav">
	<button class="std-btn std-btn-header sdt-toggle-btn" on:click|preventDefault={onSwitchView}
		>{tableCaption}</button
	>
	<div class="sdt-nav-btns">
		{#if enableTimeToggle && internalDate}
			<button
				...
				title={i18n.timeView}
				on:click|preventDefault={onTimeSwitch}
			>
				<svg
					class="sdt-svg"
		...
		<button
			...
			on:click|preventDefault={() => onTransformChangeMonth(-1)}
		>
			<svg
				class="sdt-svg"
				xmlns="http://www.w3.org/2000/svg"
...
<div class="sdt-calendar" class:is-grid={viewChanged}>
	{#if currentView === MODE_DECADE}
		<table class="sdt-table" style="max-height: 221px; height: 221px">
			<tbody
				in:swapTransition={{ duration, start, opacity: 1 }}
				class="sdt-tbody-lg"
				out:swapTransition|local={{ duration, end, start: 1 }}
				on:outroend={onTransitionOut}
			>
	...
	{#if currentView === MODE_MONTH}
		<table class="sdt-table sdt-table-height">
			<tbody
				in:swapTransition={{ duration, start: 0.5, opacity: 1 }}
				out:swapTransition|local={{ duration, start: Math.abs(viewDelta) }}
				on:outroend={onTransitionOut}
			>
				<tr class="sdt-cal-td">
						...
						{#each row as currDate, j (j)}
							...
							>
								<button
									...
									disabled={isDisabledDate(currDate)}>{currDate.getUTCDate()}</button
								>
							</td>
						{/each}
					</tr>
